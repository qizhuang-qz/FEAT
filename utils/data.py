...
from utils.DomainNet import build_merged_domainnet
import os
import ipdb
...
class DomainNet345(iData):   # 200*500=10w, 5 tasks, each task=40*500=2w
    def __init__(self, args):
        super(DomainNet345, self).__init__(args)
        self.use_path = True
        self.train_trsf = [
    ...
    def download_data(self):
        ...
        train_dst, test_dst = build_merged_domainnet(root=root, domains=domains, split=0.3, seed=123, transform_train=self.train_trsf, transform_test=self.test_trsf)
        # ipdb.set_trace()
        self.train_data, self.train_targets = np.array(train_dst.items)[:, 0], np.array(train_dst.items)[:, 1].astype(int)
        self.test_data, self.test_targets = np.array(test_dst.items)[:, 0], np.array(test_dst.items)[:, 1].astype(int)
