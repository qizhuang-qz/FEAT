...
class BaseLearner(object):
    ...
    def combine_dataset(self, pre_dataset, cur_dataset, size):
        ...
        if size !=0:
            print(idx)
            print("size: ", size, "len(pre_dataset.idxs): ", len(pre_dataset.idxs))
            ...
            # combined_label = np.concatenate(combined_label)
        idata = _get_idata(self.dataset_name, self.args)
        _train_trsf, _common_trsf = idata.train_trsf, idata.common_trsf
        trsf = transforms.Compose([*_train_trsf, *_common_trsf])      
        combined_dataset = DummyDataset(combined_data, combined_label, trsf, use_path=False)
    ...
    def eval_task(self, args, old_classes, P_H=None, P_T=None, bar_eH_G: float = None, r_H: int = None, r_T: int = None):
        y_pred, y_true = self._eval_cnn(self.test_loader, old_classes, P_H, P_T, bar_eH_G, r_H, r_T)
    ...
    def _compute_accuracy(self, model, loader, P_H=None, P_T=None, bar_eH_G: float = None, r_H: int = None, r_T: int = None, old_classes=0):
        model.eval()
        correct, total = 0, 0
        ...
        for i, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.cuda()
            with torch.no_grad():
                outs = model(inputs, P_H, P_T, bar_eH_G, r_H, r_T)
                outputs = outs["logits"]
                # out_0, out_1 = outs["cos_H"], outs["cos_T"]
                # out_0_all.append(out_0)
                # out_1_all.append(out_1)
                # targets_all.extend(targets) 

            predicts = torch.max(outputs, dim=1)[1]
            # predicts = torch.max(out_0, dim=1)[1]
            correct += (predicts.cpu() == targets).sum()
            total += len(targets)
            
        # out_0_all = torch.cat(out_0_all).cpu().numpy()
        # out_1_all = torch.cat(out_1_all).cpu().numpy()
        # targets_all = torch.tensor(targets_all).cpu().numpy()
        
        # np.save('cos_H.npy', out_0_all)
        # np.save('cos_T.npy', out_1_all)
        # np.save('labels.npy', targets_all)    
        
        return np.around(tensor2numpy(correct) * 100 / total, decimals=2)

    def _eval_cnn(self, loader, old_classes=0, P_H=None, P_T=None, bar_eH_G: float = None, r_H: int = None, r_T: int = None, ):
        self._network.eval()
        y_pred, y_true = [], []
        for _, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.cuda()
            with torch.no_grad():
                outputs = self._network(inputs, P_H, P_T, bar_eH_G, r_H, r_T)["logits"]
